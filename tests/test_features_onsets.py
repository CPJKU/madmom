...
spec = Spectrogram(AUDIO_PATH + 'sample.wav', circular_shift=True)
log_filt_spec = LogarithmicFilteredSpectrogram(spec, num_bands=24,
                                               mul=1, add=1)

superflux_act = np.load(ACTIVATIONS_PATH + 'sample.superflux.npz')
act = superflux_act['activations']
fps = superflux_act['fps']


class TestHighFrequencyContentFunction(unittest.TestCase):

    def test_values(self):
        odf = high_frequency_content(log_filt_spec)
...
class TestSpectralDiffFunction(unittest.TestCase):
    ...
    def test_values(self):
        odf = spectral_diff(log_filt_spec)
...
class TestSpectralFluxFunction(unittest.TestCase):
    ...
    def test_values(self):
        odf = spectral_flux(log_filt_spec)
...
class TestComplexFluxFunction(unittest.TestCase):
    ...
    def test_values(self):
        odf = complex_flux(log_filt_spec)
...
class TestModifiedKullbackLeiblerFunction(unittest.TestCase):
    ...
    def test_values(self):
        odf = modified_kullback_leibler(log_filt_spec)
...
class TestPhaseDeviationFunction(unittest.TestCase):
    ...
    def test_values(self):
        odf = phase_deviation(log_filt_spec)
...
class TestRectifiedComplexDomainFunction(unittest.TestCase):
    ...
    def test_errors(self):
        with self.assertRaises(ValueError):
            rectified_complex_domain(log_filt_spec)
...
class TestPeakPickingFunction(unittest.TestCase):
    ...
    def test_values(self):
        onsets = peak_picking(act, 1.1)
        self.assertTrue(np.allclose(onsets[:6], [2, 10, 17, 48, 55, 80]))
        # smooth
        onsets = peak_picking(act, 1.1, smooth=3)
        self.assertTrue(np.allclose(onsets[:6], [2, 10, 17, 24, 48, 55]))
        # default values
        onsets = peak_picking(act, 1.1, pre_max=2, post_max=10, pre_avg=30)
