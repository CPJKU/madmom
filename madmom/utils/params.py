...
def add_audio_arguments(parser, online=None, norm=NORM, att=ATT, fps=FPS,
                        window=FRAME_SIZE):
    """
    ...
    if online is not None:
        group.add_argument('--online', dest='online', action='store_true',
                           default=online,
                           help='operate in online mode [default=%s]' % online)
    if norm is not None:
        group.add_argument('--norm', action='store_true', default=norm,
                           help='normalize the audio signal '
                                '(switches to offline mode)')
    if att is not None:
        group.add_argument('--att', action='store', type=float, default=att,
                           help='attenuate the audio signal [dB]')
    if fps is not None:
        group.add_argument('--fps', action='store', type=int, default=fps,
                           help='frames per second [default=%i]' % fps)
    if window is not None:
        group.add_argument('--window', action='store', type=int,
                           default=window,
                           help='frame length [samples, default=%i]' % window)
...
def add_spec_arguments(parser, ratio=RATIO, diff_frames=DIFF_FRAMES):
    ...
    group.add_argument('--diff_frames', action='store', type=int,
                       default=diff_frames,
                       help='diff frames [default=%s]' % diff_frames)
    group.add_argument('--ratio', action='store', type=float, default=ratio,
                       help='window magnitude ratio to calc number of diff '
                            'frames [default=%.1f]' % ratio)
    # return the argument group so it can be modified if needed
    return group
    ...
    if filtering is not None:
        group.add_argument('--filter', action='store_true', default=False,
                           help='filter the magnitude spectrogram with a '
                                'filterbank [default=False]')
    if bands is not None:
        group.add_argument('--bands', action='store', type=int, default=bands,
                           help='number of bands per octave '
                                '[default=%i]' % bands)
    if fmin is not None:
        group.add_argument('--fmin', action='store', type=float, default=fmin,
                           help='minimum frequency of filter in Hz '
                                '[default=%i]' % fmin)
    if fmax is not None:
        group.add_argument('--fmax', action='store', type=float, default=fmax,
                           help='maximum frequency of filter in Hz '
                                '[default=%i]' % fmax)
    if norm_filters is False:
        # switch to turn it on
        group.add_argument('--norm_filters', action='store_true',
                           default=norm_filters,
                           help='normalize filters to have equal area')
    if norm_filters is True:
        group.add_argument('--no_norm_filters', dest='norm_filters',
...
def add_log_arguments(parser, log=None, mul=MUL, add=ADD):
    ...
    if log is not None:
        group.add_argument('--log', action='store_true', default=log,
                           help='logarithmic magnitude [default=%s]' % log)
    if mul is not None:
        group.add_argument('--mul', action='store', type=float, default=mul,
                           help='multiplier (before taking the log) '
                                '[default=%i]' % mul)
    if add is not None:
        group.add_argument('--add', action='store', type=float, default=add,
...
def add_spectral_odf_arguments(parser, method='superflux', methods=None,
                               max_bins=MAX_BINS):
    """
    ...
    # add spec related options to the existing parser
    # spectrogram options
    group = parser.add_argument_group('spectral onset detection function '
                                      'arguments')
    if methods is not None:
        group.add_argument('-o', dest='odf', default=method,
                           help='use one of these onset detection functions '
                                '(%s) [default=%s]' % (methods, method))
    #if 'superflux' in methods or method == 'superflux':
    group.add_argument('--max_bins', action='store', type=int,
                       default=max_bins,
...
def add_onset_arguments(parser, threshold=THRESHOLD, smooth=SMOOTH,
    ...
    group.add_argument('-t', dest='threshold', action='store', type=float,
                       default=threshold,
                       help='detection threshold [default=%.2f]' % threshold)
    group.add_argument('--smooth', action='store', type=float, default=smooth,
                       help='smooth the onset activations over N seconds '
                            '[default=%.2f]' % smooth)
    group.add_argument('--combine', action='store', type=float,
                       default=combine,
                       help='combine onsets within N seconds '
                            '[default=%.2f]' % combine)
    group.add_argument('--pre_avg', action='store', type=float,
                       default=pre_avg,
                       help='build average over N previous seconds '
                            '[default=%.2f]' % pre_avg)
    group.add_argument('--post_avg', action='store', type=float,
                       default=post_avg,
                       help='build average over N following seconds '
                            '[default=%.2f]' % post_avg)
    group.add_argument('--pre_max', action='store', type=float,
                       default=pre_max,
                       help='search maximum over N previous seconds '
                            '[default=%.2f]' % pre_max)
    group.add_argument('--post_max', action='store', type=float,
                       default=post_max,
                       help='search maximum over N following seconds '
                            '[default=%.2f]' % post_max)
    group.add_argument('--delay', action='store', type=float, default=delay,
                       help='report the beats N seconds delayed [default=%i]'
                       % delay)
    # return the argument group so it can be modified if needed
    return group


def add_beat_arguments(parser, threshold=BT, smooth=BS, min_bpm=MIN_BPM,
                       max_bpm=MAX_BPM):
    """
    ...
    group.add_argument('-t', dest='threshold', action='store', type=float,
                       ...
                       help='detection threshold '
                            '[default=%.2f]' % threshold)
    group.add_argument('--smooth', action='store', type=float, default=smooth,
                       help='smooth the onset activations over N seconds '
                            '[default=%.2f]' % smooth)
    group.add_argument('--min_bpm', action='store', type=float,
                       default=min_bpm,
                       help='minimum tempo [bpm, default=%.2f]' % min_bpm)
    group.add_argument('--max_bpm', action='store', type=float,
                       default=max_bpm,
                       help='maximum tempo [bpm, default=%.2f]' % max_bpm)
    # return the argument group so it can be modified if needed
    return group


def add_nn_arguments(parser, threads=2, nc_file=tempfile.mkstemp()[1],
                     nn_files=None):
    """
    Add beat tracking related arguments to an existing parser object.
    ...
    group.add_argument('--threads', action='store', type=int, default=threads,
                       help='number of threads [default=2]')
    group.add_argument('--nc_file', action='store', type=str, default=nc_file,
                       help='temporary file to use')
    group.add_argument('--nn_files', action='append', type=str,
                       default=nn_files,
...
def add_io_arguments(parser):
    ...
    """
    # add onset detection related options to the existing parser
    group = parser.add_argument_group('save/load the activations')
    # add options for saving and loading the activations
    group.add_argument('-s', dest='save', action='store_true', default=False,
                       help='save the activations to file')
    group.add_argument('-l', dest='load', action='store_true', default=False,
                       help='load the activations from file')
    group.add_argument('--sep', action='store', default='',
...
def parser():
    ...
    onset.add_argument('--not_needed', action='store_true', default=True,
